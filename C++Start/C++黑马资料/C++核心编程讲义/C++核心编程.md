...
//2. 声明和实现只能有一个有默认参数
//     如果函数声明有默认值，函数实现的时候就不能有默认参数  反之亦然
...
int main() {
    //空对象占用内存空间为:1
    //C++编译器会给每个空对象也分配一个字节空间，是为了区分空对象而占用的内存位置
...
public:
    ...
    void ShowPerson() {
        ...
        cout << mAge << endl;//mAge属性前默认加了 this-> 
    }
...
{
    Person * p = NULL;
    p->ShowClassName(); //空指针，可以调用成员函数 this指针与p相同为NULL
    p->ShowPerson();  //但是如果成员函数中用到了this指针，就不可以了 因为this为NULL
}
...
public:
    ...
    //使this由Person *const this; 变为 const Person *const this;
...
存在友元成员函数的类放在要访问的类前/声明在前
友元访问主类私有成员的友元成员函数写做类外函数形式(在class内声明,写在class外加 xxx:: 保证作用域)
